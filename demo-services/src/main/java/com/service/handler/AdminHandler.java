...
public class AdminHandler extends BaseRequestHandler {
	...
	public BaseResponse getuserlist() {
		...
		List<AdminlistDTO> list = adminService.getuserlist();
	...
	public BaseResponse deleteUser(Integer adminId) {
		try {

			if (adminId == null ){

				return convertAdmintoresponse(Constants.STATUSCODE_104, "parameter ListId can't Be Null or Empty", null,
						null, null);
			}

			Boolean deleteUser = adminService.deleteUser(adminId);

			if (deleteUser) {
				return convertAdmintoresponse(Constants.STATUSCODE_100, Constants.DELETE_SUCCESS, null, deleteUser,
						null);

			} else {
				return convertAdmindeletetoresponse(Constants.STATUSCODE_103, Constants.NORECORDFOUND, "", null, null);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return convertAdmindeletetoresponse(Constants.STATUSCODE_104, Constants.OPERATIONAL_ERROR, null, null,
					null);
		}
	}
}
